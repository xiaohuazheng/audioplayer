...
;(function($, window, document, undefined) {
	// 播放器样式 - 替换到css更改样式即可
	if (!$('#ppq-player-style').length) {
		var style = '<style id="ppq-player-style" type="text/css">\
			html{font-size:14px}\
			...
			.play-pause-btn .play-pause-icon:after{position:absolute;top:50%;left:50%;-webkit-transform:translate(-50%,-50%);-ms-transform:translate(-50%,-50%);transform:translate(-50%,-50%)}\
			.play-pause-btn .play-pause-icon:after,.player-playing .play-pause-icon:after{background-image:url(data:image/png;base64,);background-size:37px 18px}\
			.ppq-player{line-height:111px;position:relative;overflow:hidden;height:111px;margin:0 auto;background:#f4f4f4}\
			.ppq-player audio{position:absolute;vertical-align:baseline}.play-pause-btn{float:left;margin:29px 0 0 12px}\
			.play-pause-btn .play-pause-icon{position:relative;display:block;width:47px;height:47px;border:3px solid #00a293;border-radius:100%;background-color:#fff}\
			.play-pause-btn .play-pause-icon:after{display:block;content:"";background-position:0 0;width:17px;height:18px}\
			.player-playing .play-pause-icon:after{background-position:-25px 0;width:12px;height:17px}\
			.player-time{float:left;width:51px;margin-right:8px;text-align:right}\
			.player-time-duration{float:right;margin:0 0 0 8px;text-align:left}\
			.player-bar{position:relative;overflow:hidden;height:5px;margin-top:53px;background-color:#fff}\
			.player-bar .player-bar-loaded{position:absolute;top:0;left:0;width:100%;height:100%;border-radius:3px;background:#ddd}\
			.player-bar .player-bar-played{position:absolute;top:0;left:0;width:0;height:100%;border-radius:3px;background:#00a293}\
			</style>';
		$('body').prepend(style);
	}
	...
	$.fn.playerInit = function() {
		...
		this.each(function() {
			...
			// 添加播放器盒子
			var $player = $('<div class="ppq-player">' + $('<div>').append($this.eq(0).clone()).html() + '<div class="play-pause-btn"><a href="javascript: void(0);" class="play-pause-icon"></a></div></div>'),
